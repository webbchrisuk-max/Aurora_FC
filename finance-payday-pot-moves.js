(() => {
  ...
  const BUILD = '20260823-finance-payday-pot-moves-1';
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      .payday-pot-moves{border:1px solid rgba(110,231,255,.14);border-radius:22px;padding:21px;background:linear-gradient(180deg,rgba(7,24,38,.96),rgba(3,13,25,.98));margin-top:0}
      ...
      .payday-pot-moves-head h4{margin:7px 0 0;font:900 23px/1.1 system-ui}
      ...
      .payday-pot-move.required{border-color:rgba(255,213,107,.16)}.payday-pot-move.required>i{background:rgba(255,213,107,.08);color:#ffd76f}
      .payday-pot-move.holding{border-color:rgba(99,245,162,.16)}.payday-pot-move.holding>i{background:rgba(99,245,162,.08);color:#99f6bc}
  ...
  function ensureHost() {
    const dashboard = document.querySelector('#paydayPanel .payday-command-dashboard');
    if (!dashboard) return null;
    let host = dashboard.querySelector('[data-payday-pot-moves]');
    if (host) return host;
    host = document.createElement('article');
    host.className = 'payday-pot-moves';
    host.dataset.paydayPotMoves = '1';
    const grid = dashboard.querySelector('.payday-dashboard-grid');
    if (grid) dashboard.insertBefore(host, grid);
    else dashboard.appendChild(host);
    return host;
  }

  function moveRows(state, c) {
    const planRows = arr(c?.fundingPlan?.rows);
    const moves = planRows
      .map(row => ({
        type:'pot',
    ...
    if (hp && holdingMove > .009) {
      ...
      moves.unshift({
        type:'holding',
        name:hp.name || 'Holding Pot',
        amount:holdingMove,
        required:holdingMove,
        reason:reasons.join(' + '),
        deadline:''
      });
    }
  ...
  function render() {
    ...
    if (!c) {
      host.innerHTML = '<div class="payday-pot-moves-empty">Waiting for the Finance payday funding engine…</div>';
      return false;
    }

    const rows = moveRows(state,c);
    const fundingPlan = c?.fundingPlan || {};
