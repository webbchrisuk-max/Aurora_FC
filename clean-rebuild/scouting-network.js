(() => {
  'use strict';
  const BUILD='20260926-scouting-network-9-research-not-blocked';
  const $=id=>document.getElementById(id);
  const num=v=>{const n=Number(String(v??'').replace(/[^0-9.-]/g,''));return Number.isFinite(n)?n:0};
  const upper=v=>String(v||'').trim().toUpperCase();
  const esc=v=>String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const pct=v=>`${num(v).toFixed(2)}%`;
  const executionProfile=row=>window.AuroraScoutingExecutionProfiles?.resolve?.(row)||row;
  const effectiveYield=row=>window.AuroraScoutingExecutionProfiles?.effectiveYieldPct?.(row)??Math.max(0,num(row?.yieldPct));
  ...
  const analystTarget=row=>nativeMoney(row?.analystPriceTargetNative,row?.analystPriceTargetCurrency||row?.executionCurrency||row?.currency);
  ...
  function tierFor(verdict,score){
    ...
    if(v==='STRONG BUY')return'PREMIER';
    if(v==='BUY'&&s>=74)return'ELITE';
    if(v==='BUY')return'READY';
    return'WATCH';
  }
  function tierRank(tier){return tier==='PREMIER'?0:tier==='ELITE'?1:tier==='READY'?2:3}
  ...
  function rankingBase(state){
    ...
    return base.map(input=>{
      ...
      let verdict=hardBlock?'BLOCKED':'WATCH';
      ...
      else if(!hardBlock)verdict='WATCH';
      ...
      const buyReady=['BUY','STRONG BUY'].includes(verdict)&&complete&&!hardBlock&&!decision.watch&&!valuationCaution&&r!=='UNKNOWN';
      const tier=buyReady?tierFor(verdict,score):'WATCH';
      return{...row,networkScore:Number(score.toFixed(1)),score:Number(score.toFixed(1)),risk:r,upsidePct:up,valuationScore,riskScore,dataQuality,stage,verdict,buyReady,tier,evidenceComplete:complete,evidenceStatus,readiness,missingEvidence:missing,decisionState:decision};
    }).sort((a,b)=>tierRank(a.tier)-tierRank(b.tier)||b.networkScore-a.networkScore||b.yieldPct-a.yieldPct||a.ticker.localeCompare(b.ticker));
  }
  function rankings(state){return rankingBase(state)}
  function coverage(state,rows){const counts=state.scouting?.universeCounts||{};const sourceRows=num(counts.watchlist)+num(counts.global)+num(counts.scout);const unique=rows.length;const validYield=rows.filter(x=>x.yieldPct>0).length;const fullData=rows.filter(x=>x.evidenceComplete).length;const lowRisk=rows.filter(x=>x.risk==='LOW').length;const top=rows.filter(x=>x.buyReady).length;const premier=rows.filter(x=>x.tier==='PREMIER').length;const elite=rows.filter(x=>x.tier==='ELITE').length;const ready=rows.filter(x=>x.tier==='READY').length;const researchQueue=rows.filter(x=>!x.buyReady&&!x.decisionState?.block).length;const gems=rows.filter(x=>!x.held&&x.buyReady&&x.yieldPct>=5).length;const vetoed=rows.filter(x=>x.decisionState?.block).length;return{sourceRows,unique,validYield,fullData,lowRisk,top,premier,elite,ready,researchQueue,gems,vetoed}}
  ...
  function openDrawer(ticker){
    ...
    reasons.push(`${row.yieldPct.toFixed(3)}% forward yield`);
    if(brokerLabel(row)!=='Broker review')reasons.push(`${brokerLabel(row)} execution route`);
    if(num(row.brokerYieldPct)>0)reasons.push(`${num(row.brokerYieldPct).toFixed(3)}% broker yield`);
    ...
    c.innerHTML=`<p class="eyebrow">CHIEF SCOUT REPORT</p><h2>${esc(row.ticker)}</h2><p>${esc(row.name||row.ticker)} · ${esc(row.sector||'Sector unclassified')}</p><div class="scout-drawer-grid"><div class="scout-drawer-stat"><span>NETWORK SCORE</span><strong>${row.networkScore.toFixed(1)}/100</strong></div><div class="scout-drawer-stat"><span>PIPELINE</span><strong>${esc(row.stage)}</strong></div><div class="scout-drawer-stat"><span>FORWARD YIELD</span><strong>${num(row.yieldPct).toFixed(3)}%</strong></div><div class="scout-drawer-stat"><span>BUY ACCOUNT</span><strong>${esc(brokerLabel(row))}</strong></div><div class="scout-drawer-stat"><span>MARKET SYMBOL</span><strong>${esc(row.marketSymbol||row.ticker)}</strong></div><div class="scout-drawer-stat"><span>BROKER YIELD</span><strong>${num(row.brokerYieldPct)>0?`${num(row.brokerYieldPct).toFixed(3)}%`:'—'}</strong></div><div class="scout-drawer-stat"><span>IG SNAPSHOT BUY</span><strong>${nativeMoney(row.brokerBuyPriceNative,row.executionCurrency||row.currency)}</strong></div><div class="scout-drawer-stat"><span>ANALYST TARGET</span><strong>${analystTarget(row)}</strong></div><div class="scout-drawer-stat"><span>ANALYST VIEW</span><strong>${esc(row.analystView||'—')}</strong></div><div class="scout-drawer-stat"><span>BUY STRENGTH</span><strong>${num(row.buyStrength).toFixed(0)}/100</strong></div><div class="scout-drawer-stat"><span>FAIR VALUE UPSIDE</span><strong>${hasValuation(row)?`${row.upsidePct.toFixed(1)}%`:'—'}</strong></div><div class="scout-drawer-stat"><span>PAYOUT RISK</span><strong>${esc(row.risk)}</strong></div><div class="scout-drawer-stat"><span>READINESS</span><strong>${esc(row.readiness)}</strong></div><div class="scout-drawer-stat"><span>DATA QUALITY</span><strong>${row.dataQuality.toFixed(0)}%</strong></div><div class="scout-drawer-stat"><span>PORTFOLIO EXPOSURE</span><strong>${num(row.exposurePct).toFixed(1)}%</strong></div><div class="scout-drawer-stat"><span>SECTOR EXPOSURE</span><strong>${num(row.sectorExposurePct).toFixed(1)}%</strong></div></div><h3>Why Aurora ranks it here</h3><p>${esc(reasons.join(' · '))}.</p><h3>Evidence source</h3><p>${esc(sourceText)}${row.evidenceUpdatedAt?` · source updated ${esc(row.evidenceUpdatedAt)}`:''}</p><div class="scout-score-bar"><div style="width:${row.networkScore}%"></div></div><h3>Verdict</h3><span class="scout-verdict ${row.verdict==='STRONG BUY'?'strong':row.verdict==='BUY'?'buy':row.verdict==='WATCH'?'watch':'block'}">${row.verdict}</span><p>${['BUY','STRONG BUY'].includes(row.verdict)?'Evidence and decision gates passed; eligible for the payday recruitment pool.':row.verdict==='WATCH'?`Not eligible for payday allocation yet · ${esc(row.readiness)}.`:'Blocked only by an explicit decision veto or high payout-risk gate.'}</p>`;
