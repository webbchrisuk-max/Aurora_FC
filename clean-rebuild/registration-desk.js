(() => {
  'use strict';
  const BUILD='20260924-registration-command-board-3-security-map';
  ...
  const plannedBroker=leg=>accountLabel(leg?.lockedAccount||leg?.account||leg?.broker||leg?.preferredBroker||leg?.platform||'');
  ...
  function render(){
    ...
    const rows=ctx.legs.map((leg,i)=>{const rec=receiptFor(ctx,leg),st=statusFor(ctx,leg,rec),diff=rec?num(rec.totalCostGbp)-num(leg.amount):0,broker=rec?accountLabel(rec.account):plannedBroker(leg),base=upper(leg.underlyingTicker||leg.ticker),exec=upper(leg.executionTicker||leg.ticker),tickerLabel=exec&&exec!==base?`${base} → ${exec}`:exec;return `<tr data-leg-id="${esc(leg.legId)}"><td><span class="ticker">${esc(tickerLabel)}</span><br><small>${esc(leg.name||leg.ticker||'')}${leg.executionMarket?` · ${esc(leg.executionMarket)}`:''}${leg.executionCurrency?` · ${esc(leg.executionCurrency)}`:''}</small></td><td>#${i+1}</td><td>${isCashLeg(leg)?`${esc(plannedBroker(leg))} cash`:'Finance'}</td><td>${money(leg.amount)}</td><td>${esc(broker||'—')}</td><td>${rec?shares(rec.shares):'—'}</td><td>${rec?money(rec.totalCostGbp):'—'}</td><td class="${diff>0?'registration-negative':diff<0?'registration-positive':''}">${rec?`${diff>0?'+':''}${money(diff)}`:'—'}</td><td>${money(leg.expectedAnnualIncome||rec?.expectedAnnualIncomeGbp||0)}/yr</td><td><span class="registration-state ${st[1]}">${st[0]}</span></td></tr>`}).join('');
