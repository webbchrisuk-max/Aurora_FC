(() => {
  ...
  const BUILD='20260826-payday-chain-health-1';
  const EPS=0.01;
  ...
  function snapshotChecks(state){
    ...
    const txIds=missionReceipts.map(r=>String(r.transactionId||'')).filter(Boolean),receiptTotal=sum(missionReceipts,'totalCostGbp');
    const settled=missionReceipts.filter(r=>!!r.settledAt);
    const checks=[
  ...
  function buildDryPlan(state,budget){
    ...
    const allocations=items.filter(r=>r.amount>.004).map((r,i)=>({legId:`DRY-LEG-${i+1}`,ticker:r.ticker,name:r.name,amount:round(r.amount),yieldPct:num(r.yieldPct),expectedAnnualIncome:round(r.amount*num(r.yieldPct)/100)}));let allocated=sum(allocations,'amount');const delta=round(budget-allocated);if(allocations.length&&Math.abs(delta)>=.01)allocations[0].amount=round(allocations[0].amount+delta);allocated=sum(allocations,'amount');
    return{ok:true,strategy,allocations,allocated};
  }

  function dryRun(){
    const A=window.AuroraClean;if(!A)return;const realBefore=JSON.stringify(A.readState()),test=clone(A.readState()),stage5=test.finance?.stage5PaydayDecision,budget=round(stage5?.maximumSafeRelease);
    const lines=[];const pass=(label,ok,detail='')=>lines.push({label,ok,detail});
    ...
    const unique=new Set(test.registration.receipts.map(r=>r.transactionId)).size===test.registration.receipts.length;pass('Duplicate guard',unique);
    ...
    const unchanged=JSON.stringify(A.readState())===realBefore;pass('Real Aurora state was not changed',unchanged,'In-memory rehearsal only');renderDry(lines,lines.every(x=>x.ok));
  }
  ...
  function boot(){if(!window.AuroraClean){setTimeout(boot,50);return}render();$('runPaydayDryRun')?.addEventListener('click',dryRun);window.addEventListener('aurora-clean:state',render);window.AuroraPaydayHealth=Object.freeze({BUILD,snapshotChecks,dryRun});}
