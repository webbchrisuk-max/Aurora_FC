(() => {
  ...
  function routeContext(){
    ...
    if(['COMPLETE','CANCELLED'].includes(upper(mission.status)))return null;
  ...
  function validation(ctx,leg,a){
    ...
    const account=accountCode(a.account),locked=lockedBroker(leg);
    if(!account)errors.push('A broker must be assigned in Transfer before execution.');
    if(locked&&account!==locked)errors.push(`This route leg is locked to ${accountLabel(locked)}.`);
    ...
    if(receiptFor(ctx,leg))errors.push('This locked leg already has a confirmed receipt.');
    if(isCashLeg(leg)){
      if(a.totalCostGbp>round(leg.amount)+EPS)errors.push(`Actual cost ${money(a.totalCostGbp)} exceeds this ${accountLabel(locked)} cash allocation ${money(leg.amount)}.`);
    }else{
      const remaining=round(num(ctx?.mission?.budget)-financeConfirmed(ctx));
      if(a.totalCostGbp>remaining+EPS)errors.push(`Actual cost ${money(a.totalCostGbp)} exceeds remaining Finance mission cash ${money(remaining)}.`);
    }
    return errors;
  }
  ...
  function render(){
    ...
    if(!ctx){
      if(status)status.textContent='No locked Transfer route yet — complete Transfer first';
      if(list)list.innerHTML='<li>No locked route.</li>';
      if(select)select.innerHTML='<option>No locked route</option>';
      if(confirmBtn)confirmBtn.disabled=true;if(settle)settle.disabled=true;
      if($('registrationReceipts'))$('registrationReceipts').textContent='0 receipt(s) recorded for this mission';
      if($('regAccount')){$('regAccount').disabled=false;$('regAccount').title='';}
      return;
    }
    ...
    if($('regVariance'))$('regVariance').textContent=`${difference>0?'+':''}${money(difference)}`;
    if($('regRemaining'))$('regRemaining').textContent=isCashLeg(leg)?`${money(planned)} ${accountLabel(lockedBroker(leg))} cash leg`:money(remaining);
    if(confirmBtn)confirmBtn.disabled=!!receiptFor(ctx,leg)||!leg||!lockedBroker(leg);
    ...
    if($('registrationReceipts'))$('registrationReceipts').textContent=`${ctx.receipts.length} receipt(s) · ${money(all)} total purchases`;
    if($('regMission'))$('regMission').textContent=`LOCKED ROUTE · Finance ${money(ctx.mission.budget)} · ${ctx.mission.id}`;
  }
  ...
  async function confirmExecution(){
    ...
    try{
      ...
      const actualCost=round(result.transaction.totalCostGbp??a.totalCostGbp);
      if(isCashLeg(leg)){
        const cashDebit=await client.post('adjustBrokerCash',{account,changeGbp:-actualCost,reference:`BUY:${transactionId}:BROKER-CASH`,note:`Registration broker-cash purchase · ${ticker} · route ${ctx.route.id}`});
