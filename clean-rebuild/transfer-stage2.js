(() => {
  ...
  const BUILD='20260911-transfer-funding-plan-6-1000-cap';
  ...
  function financePlan(state){
    ...
    let allocated=round(allocations.reduce((s,r)=>s+r.amount,0)),delta=round(budget-allocated);
    if(allocations.length&&Math.abs(delta)>=.01)allocations[0].amount=round(allocations[0].amount+delta);
  ...
  function assignedCashTotal(base,usable){
    let total=0;
    if((base||[]).some(r=>brokerCode(r)==='IG'))total+=Number(usable.IG||0);
    if((base||[]).some(r=>brokerCode(r)==='T212'))total+=Number(usable.T212||0);
    return round(total);
  }

  function financeContribution(fullBudget,eligibleBrokerCash){
    const availableWhole=Math.floor(Number(fullBudget||0)+1e-9);
    if(availableWhole<=0)return 0;
    if(Number(fullBudget||0)+Number(eligibleBrokerCash||0)<=TARGET_BUYING_POWER_GBP)return availableWhole;
    const needed=Math.max(0,TARGET_BUYING_POWER_GBP-Number(eligibleBrokerCash||0));
    return Math.min(availableWhole,Math.ceil(needed-1e-9));
  }

  function scaleFinancePlan(plan,newBudget){
    const budget=Math.max(0,Math.floor(Number(newBudget||0)));
    if(!plan?.allocations?.length)return plan;
    if(budget===0){
      const allocations=plan.allocations.map(r=>({...r,amount:0,expectedAnnualIncome:0}));
      return{...plan,budget,allocations,allocated:0,expectedAnnualIncome:0};
    }
    const sourceTotal=plan.allocations.reduce((s,r)=>s+Number(r.amount||0),0)||1;
    let used=0;
    const allocations=plan.allocations.map((r,i)=>{
      const amount=i===plan.allocations.length-1?round(budget-used):round(budget*(Number(r.amount||0)/sourceTotal));
      used=round(used+amount);
      return{...r,amount,expectedAnnualIncome:round(amount*Number(r.yieldPct||0)/100)};
    });
    return{...plan,budget,allocations,allocated:round(allocations.reduce((s,r)=>s+r.amount,0)),expectedAnnualIncome:round(allocations.reduce((s,r)=>s+r.expectedAnnualIncome,0))};
  }

  function brokerTransferPlan(base,budget,fullBudget){
    ...
    const wholeBudget=Math.max(0,Math.floor(Number(budget||0)+1e-9));
    ...
    return{IG:plan.IG,T212:plan.T212,total,untransferred:round(Number(fullBudget??budget||0)-total),exact:grouped};
  }
  ...
  function distributeCash(account,amount,base,missionId){
    ...
    const demand=eligible.reduce((s,r)=>s+Math.max(0,Number(r.amount||0)),0)||eligible.length;
    let used=0;
    return eligible.map((r,i)=>{
      const weight=demand===eligible.length?1:Math.max(0,Number(r.amount||0));
      const a=i===eligible.length-1?round(total-used):round(total*(weight/demand));
      used=round(used+a);
      return{legId:`LEG-${hash(`${missionId}|BROKER_CASH|${account}|${i}|${r.ticker}|${a}`)}`,ticker:r.ticker,name:r.name,yieldPct:r.yieldPct,score:r.score,selectionRank:r.selectionRank,amount:a,expectedAnnualIncome:round(a*r.yieldPct/100),fundingSource:'BROKER_CASH',lockedAccount:account,account};
    }).filter(r=>r.amount>0);
  }
  ...
  function fundedPlan(state){
    ...
    const eligibleCash=assignedCashTotal(raw.allocations,usable);
    const financeUsed=financeContribution(raw.budget,eligibleCash);
    const f=scaleFinancePlan(raw,financeUsed);
    ...
    const transfers=brokerTransferPlan(f.allocations,f.budget,raw.budget);
    const financeLeftBehind=round(raw.budget-f.allocated);
    return{...f,financeMissionBudget:raw.budget,financeUsed:f.allocated,financeLeftBehind,brokerCashAllocations:extra,brokerCashObserved:observed,brokerCash:usable,brokerCashThreshold:BROKER_CASH_MIN_GBP,targetBuyingPower:TARGET_BUYING_POWER_GBP,brokerCashTotal,transferPlan:transfers,totalBuyingPower:round(f.allocated+brokerCashTotal),totalExpectedAnnualIncome:round(f.expectedAnnualIncome+extra.reduce((s,r)=>s+r.expectedAnnualIncome,0))};
  }

  async function refreshCash(){
    const client=window.AuroraData2Client;
    if(!client?.post){cash=readCache();rebuildBrokerCash();render();return}
    try{const r=await client.post('brokerCashSnapshot',{});if(!r?.balances)throw new Error('Incomplete broker cash snapshot');cash=r;writeCache(r)}catch(_){cash=readCache()}
  ...
  function rebuildBrokerCash(){
    ...
    A.updateState(next=>{
      const r=next.transfer?.route;if(!r||r.locked)return;
      r.allocations=p.allocations;r.brokerCashAllocations=p.brokerCashAllocations;r.brokerCashObserved=p.brokerCashObserved;r.brokerCashPlanned=p.brokerCash;r.brokerCashThreshold=p.brokerCashThreshold;r.targetBuyingPower=p.targetBuyingPower;r.brokerCashAllocated=p.brokerCashTotal;r.transferPlan=p.transferPlan;r.financeMissionBudget=p.financeMissionBudget;r.financeAllocated=p.financeUsed;r.financeLeftBehind=p.financeLeftBehind;r.totalAllocated=p.totalBuyingPower;r.expectedAnnualIncome=p.totalExpectedAnnualIncome;r.updatedAt=new Date().toISOString();
    });
    return true;
  }
  ...
  function lockRoute(){
    ...
    rebuildBrokerCash();
    A.updateState(next=>{if(!next.transfer?.route?.allocations?.length||!next.transfer?.mission||next.transfer.route.locked||!routeBrokerReady(next.transfer.route))return;next.transfer.route.locked=true;next.transfer.route.lockedAt=new Date().toISOString();next.transfer.mission.status='LOCKED';next.transfer.mission.updatedAt=new Date().toISOString();});
    render();return true;
  }

  function render(){
    const A=window.AuroraClean;if(!A)return;
    const state=A.readState(),mission=state.transfer?.mission,source=state.scouting?.allocationPlan,route=state.transfer?.route,preview=fundedPlan(state),set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v},rows=document.getElementById('transferStage2Rows'),build=document.getElementById('transferStage2Build'),lock=document.getElementById('transferStage2Lock');
    ...
    set('transferCashIG',money(observed.IG));set('transferCashT212',money(observed.T212));set('transferBuyingPower',money(preview?.totalBuyingPower||route?.totalAllocated||mission?.budget||0));
    set('transferCashIGUse',usable.IG?`Eligible · ${money(usable.IG)} will be used when an IG leg exists`:`Not used until balance reaches ${wholeMoney(BROKER_CASH_MIN_GBP)}`);
    set('transferCashT212Use',usable.T212?`Eligible · ${money(usable.T212)} will be used when a Trading 212 leg exists`:`Not used until balance reaches ${wholeMoney(BROKER_CASH_MIN_GBP)}`);
    set('transferToIG',wholeMoney(tp.IG));set('transferToT212',wholeMoney(tp.T212));set('transferPenceLeft',money(tp.untransferred));
    const used=preview?.financeUsed??route?.financeAllocated??mission?.budget??0;
    const left=preview?.financeLeftBehind??route?.financeLeftBehind??0;
    ...
    if(route?.allocations?.length){
      const all=[...(route.allocations||[]).filter(r=>Number(r.amount||0)>0),...(route.brokerCashAllocations||[])];
      ...
      if(rows)rows.innerHTML=all.map(r=>`<li><strong>${esc(r.ticker)}</strong> — ${money(r.amount)} — ${r.fundingSource==='BROKER_CASH'?`${esc(r.lockedAccount==='IG'?'IG ISA':'Trading 212 ISA')} CASH`:brokerCode(r)?esc(brokerCode(r)==='IG'?'IG ISA':'Trading 212 ISA'):'BROKER PENDING'} — projected annual income ${money(r.expectedAnnualIncome)}</li>`).join('');
  ...
  function bind(){
    ...
    render();cash=readCache();setTimeout(rebuildBrokerCash,0);refreshCash();
    window.AuroraTransferStage2=Object.freeze({BUILD,BROKER_CASH_MIN_GBP,TARGET_BUYING_POWER_GBP,financePlan,fundedPlan,brokerTransferPlan,refreshCash,render,buildRoute,rebuildBrokerCash,lockRoute,missionCanBuild,routeBrokerReady});
    return true;
  }
