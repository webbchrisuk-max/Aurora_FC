(() => {
  ...
  const BUILD='20260910-transfer-approved-plan-plus-broker-cash-4-broker-aligned-cash';
  ...
  function distributeCash(account,amount,base,missionId){
    ...
    if(!(total>0)||!eligible.length)return[];
  ...
  function rebuildBrokerCash(){
    ...
    A.updateState(next=>{const r=next.transfer?.route;if(!r||r.locked)return;r.allocations=p.allocations;r.brokerCashAllocations=p.brokerCashAllocations;r.brokerCashPlanned=p.brokerCash;r.brokerCashAllocated=p.brokerCashTotal;r.financeAllocated=p.allocated;r.totalAllocated=p.totalBuyingPower;r.expectedAnnualIncome=p.totalExpectedAnnualIncome;r.updatedAt=new Date().toISOString();});
    return true;
  }
  ...
  function render(){
    ...
    const b=cashBalances();set('transferCashIG',money(b.IG));set('transferCashT212',money(b.T212));set('transferBuyingPower',money(preview?.totalBuyingPower||mission?.budget||0));set('transferStage2Mission',mission?`${mission.status} · Finance ${money(mission.budget)}`:'No Finance mission');
  ...
  function bind(){const A=window.AuroraClean;if(!A)return false;
    ...
    window.addEventListener('aurora-clean:state',render);render();cash=readCache();setTimeout(rebuildBrokerCash,0);refreshCash();window.AuroraTransferStage2=Object.freeze({BUILD,financePlan,fundedPlan,refreshCash,render,buildRoute,rebuildBrokerCash,lockRoute,missionCanBuild,routeBrokerReady});return true;
  }
