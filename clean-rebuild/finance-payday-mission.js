(() => {
  ...
  const BUILD='20260903-payday-mission-monzo-confirmed-import-1';
  ...
  function ensureMonzoStatus(){
    const received=$('financeWagesReceived');if(!received)return null;
    let status=$('financeMonzoPaydayStatus');
    if(!status){status=document.createElement('small');status.id='financeMonzoPaydayStatus';status.className='payday-monzo-status';status.textContent='MONZO PAYDAY · checking confirmed notifications…';received.insertAdjacentElement('afterend',status)}
    return status;
  }

  function setMonzoStatus(text,kind=''){
    const el=ensureMonzoStatus();if(!el)return;
    el.className=`payday-monzo-status ${kind}`.trim();el.textContent=text;
  }
  ...
  function ensureCashFields(){
    ...
    available.readOnly=true;available.setAttribute('aria-readonly','true');
  ...
  function syncCashFieldsFromState(){
    const A=window.AuroraClean;if(!A?.readState)return;ensureCashFields();
    const state=A.readState(),carry=$('financeMainAccountLeftover');if(carry&&document.activeElement!==carry)carry.value=num(state.finance?.mainAccountLeftover).toFixed(2);
  ...
  function buildRows(state){
    const f=state.finance||{},h=f.stage3HoldingPot||{},p=f.stage4PotFunding||{},d=f.stage5PaydayDecision||{},rows=[];
    ...
    const holdingMove=num(d.baseHoldingContribution)+num(d.holdingSafetyTopUp);if(holdingMove>0)rows.push({name:'Move to Holding Pot',detail:`Required ${money(h.dynamicTarget||h.cycleRequired)} · projected after funding ${money(h.afterFunding)}.`,amount:holdingMove});
    ...
    rows.push({name:'Stage 6 · Mission release',detail:'This is the amount left after every required move above.',amount:num(d.maximumSafeRelease),type:'release'});return rows;
  }

  function render(){
    const A=window.AuroraClean;if(!A?.readState)return;ensureStyle();simplifyOldStages();syncCashFieldsFromState();const host=ensurePlan();if(!host)return;
  ...
  function rebuildFromPay(){
    const E=window.AuroraFinanceEngine,A=window.AuroraClean;if(!E||!A)return;ensureCashFields();syncAvailablePreview();const leftover=num($('financeMainAccountLeftover')?.value);
    A.updateState(s=>{s.finance.mainAccountLeftover=leftover;});syncAvailablePreview();E.commitCashTruth();E.commitBills();E.commitHolding();E.commitPots();E.commitDecision();setTimeout(render,0);
  }

  async function syncConfirmedMonzoPayday(){
    const client=window.AuroraData2Client;if(!client?.get){setMonzoStatus('MONZO PAYDAY · backend connection unavailable','wait');return}
    try{
      const result=await client.get('listMonzoPaydayNotifications',{limit:20});
      const rows=Array.isArray(result?.notifications)?result.notifications:[];
      const confirmed=rows.find(r=>String(r?.status||'').toUpperCase()==='CONFIRMED'&&num(r?.detectedAmountGbp)>0);
      if(!confirmed){const candidate=rows.find(r=>String(r?.status||'').toUpperCase()==='PAYDAY_CANDIDATE');setMonzoStatus(candidate?'MONZO PAYDAY · candidate received · awaiting confirmation':'MONZO PAYDAY · waiting for confirmed wage','wait');return}
      const eventId=String(confirmed.eventId||'');const previous=localStorage.getItem(IMPORT_KEY)||'';
      if(eventId&&eventId===previous){setMonzoStatus(`MONZO PAYDAY · imported ${money(confirmed.detectedAmountGbp)}`,'ok');return}
      const received=$('financeWagesReceived');if(!received)return;
      received.value=num(confirmed.detectedAmountGbp).toFixed(2);syncAvailablePreview();
      localStorage.setItem(IMPORT_KEY,eventId);setMonzoStatus(`MONZO PAYDAY · confirmed wage imported ${money(confirmed.detectedAmountGbp)}`,'ok');
      rebuildFromPay();
    }catch(err){setMonzoStatus(`MONZO PAYDAY · ${String(err?.message||'could not check backend')}`,'wait')}
  }

  function boot(){
    if(!window.AuroraClean||!window.AuroraFinanceEngine){setTimeout(boot,60);return}
    ensureCashFields();simplifyOldStages();render();$('financeCalculate')?.addEventListener('click',()=>setTimeout(rebuildFromPay,0));
    window.addEventListener('aurora-clean:state',render);window.addEventListener('pageshow',()=>{render();syncConfirmedMonzoPayday()});window.addEventListener('focus',syncConfirmedMonzoPayday);
    window.AuroraFinancePaydayMission=Object.freeze({BUILD,render,rebuildFromPay,syncAvailablePreview,syncConfirmedMonzoPayday});
    setTimeout(syncConfirmedMonzoPayday,250);
  }
