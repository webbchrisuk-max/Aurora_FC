(() => {
  'use strict';
  const BUILD='20260827-clean-bill-audit-1';
  ...
  function boot(){if(!window.AuroraClean){setTimeout(boot,50);return}render();window.addEventListener('aurora-clean:state',render);window.addEventListener('pageshow',render);window.AuroraFinanceBillAudit=Object.freeze({BUILD,audit,render});}
