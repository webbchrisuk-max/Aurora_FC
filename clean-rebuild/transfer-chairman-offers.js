(() => {
  ...
  const BUILD='20260827-clean-chairman-offers-5-hard-broker-eligibility';
  const AUTO_GAIN_TRIGGER_PCT=6;
  const MIN_INCOME_UPLIFT_PCT=8;
  const COOLDOWN_DAYS=90;
  const BROKER_DENY={IG:new Set(['TRIG']),T212:new Set()};
  ...
  function accountCode(v){const s=upper(v);if(s.includes('212'))return'T212';if(s==='IG'||s.includes('IG ISA'))return'IG';return''}
  function brokerTokens(row){
    const raw=[row?.brokerAvailability,row?.availableAccounts,row?.availableBrokers,row?.brokers,row?.broker,row?.accounts,row?.accountAvailability,row?.platforms,row?.platform].filter(Boolean).join(' ');
    return upper(raw).split(/[^A-Z0-9]+/).filter(Boolean);
  }
  function brokerEligible(row,sourceAccount){
    const code=accountCode(sourceAccount),ticker=upper(row?.ticker);if(!code)return false;
    if(BROKER_DENY[code]?.has(ticker))return false;
    const tokens=brokerTokens(row);if(!tokens.length)return true;
    if(code==='IG')return tokens.includes('IG');
    if(code==='T212')return tokens.includes('T212')||(tokens.includes('TRADING')&&tokens.includes('212'))||tokens.includes('212');
    return false;
  }
  ...
  function bestReplacement(state,proceeds,incomeSurrendered,excludeTicker,sourceAccount=''){
    ...
    if(!(amount>0)||!(lost>=0)||!accountCode(sourceAccount))return null;
    const candidates=scoutingRows(state).filter(r=>upper(r.ticker)!==exclude&&brokerEligible(r,sourceAccount)).map(r=>{
      const ticker=upper(r.ticker),yieldPct=Math.max(0,num(r.yieldPct)),projectedAnnualIncome=round(amount*yieldPct/100),incomeUplift=round(projectedAnnualIncome-lost),incomeUpliftPct=lost>0?(incomeUplift/lost)*100:(incomeUplift>0?100:0),heldBook=profile.tickerBook.get(ticker)||0,postTickerPct=profile.totalBook>0?((heldBook+amount)/(profile.totalBook+amount))*100:0,sector=String(r.sector||'').trim(),sectorBook=sector?(profile.sectorBook.get(sector)||0):0,postSectorPct=profile.totalBook>0&&sector?((sectorBook+amount)/(profile.totalBook+amount))*100:0,sameBroker=true,diversificationBonus=!r.held?12:Math.max(0,8-(postTickerPct/5)),concentrationPenalty=Math.max(0,postTickerPct-20)*1.4+Math.max(0,postSectorPct-35)*.6,brokerBonus=5,score=num(r.score),decisionScore=incomeUpliftPct+diversificationBonus+brokerBonus-concentrationPenalty+(score/25);
  ...
  function replacementForOffer(state,offer,m){
    ...
    const storedRow=scoutingRows(state).find(r=>upper(r.ticker)===upper(offer.replacementTicker));
    if(storedRow&&!brokerEligible(storedRow,offer.account))return null;
    if(BROKER_DENY[accountCode(offer.account)]?.has(upper(offer.replacementTicker)))return null;
  ...
  function render(){
    ...
    host.innerHTML=rows.length?rows.map(o=>{const m=offerMetrics(state,o),r=replacementForOffer(state,o,m),s=upper(o.status)||'WATCHING',canAccept=s==='ACTIVE'&&!!r&&r.incomeUpliftPct>=MIN_INCOME_UPLIFT_PCT,canWithdraw=['WATCHING','ACTIVE'].includes(s),auto=String(o.source||'').startsWith('AUTO_6_PERCENT'),routeId=o.replacementRouteId||'',verdict=r?.confidence||'NO QUALIFIED BROKER-COMPATIBLE UPGRADE';return`<article class="chairman-offer-card status-${s.toLowerCase()}"><div class="chairman-offer-head"><div><span class="chairman-status">${esc(s)}</span><h3>${esc(o.ticker)} · ${esc(o.account)}</h3><p>${esc(o.name||o.ticker)}${auto?' · AUTO +6%':''}</p></div><strong>${m.shares.toLocaleString('en-GB',{maximumFractionDigits:6})} shares</strong></div><div class="chairman-offer-grid"><div><span>LIVE PRICE</span><strong>${m.live>0?price(m.live):'—'}</strong></div><div><span>6% / TARGET PRICE</span><strong>${price(m.target)}</strong></div><div><span>LIVE GAIN</span><strong class="${m.liveGain>=0?'offer-positive':'offer-negative'}">${m.liveGain>=0?'+':''}${money(m.liveGain)} · ${m.liveGainPct.toFixed(2)}%</strong></div><div><span>SALE VALUE</span><strong>${money(m.liveValue)}</strong></div><div><span>INCOME LOST</span><strong>${money(m.incomeSurrendered)}/yr</strong></div><div><span>REPLACE WITH</span><strong>${r?`${esc(r.ticker)} · ${r.yieldPct.toFixed(2)}%`:'No qualified replacement'}</strong></div><div><span>NEW INCOME</span><strong>${r?`${money(r.projectedAnnualIncome)}/yr`:'—'}</strong></div><div><span>INCOME UPLIFT</span><strong class="${r?.incomeUplift>0?'offer-positive':''}">${r?`+${money(r.incomeUplift)}/yr · +${r.incomeUpliftPct.toFixed(1)}%`:'—'}</strong></div><div><span>POST POSITION</span><strong>${r?`${r.postTickerPct.toFixed(1)}% ticker · ${r.postSectorPct.toFixed(1)}% sector`:'—'}</strong></div><div><span>BROKER FIT</span><strong>${r?`${esc(o.account)} ✓`:'NO ELIGIBLE SAME-ACCOUNT REPLACEMENT'}</strong></div><div><span>VERDICT</span><strong class="${r?'offer-positive':''}">${esc(verdict)}</strong></div>${routeId?`<div><span>REPLACEMENT ROUTE</span><strong>${esc(routeId)} · READY</strong></div>`:''}</div><div class="chairman-offer-actions">${canAccept?`<button type="button" class="finance-primary" data-offer-accept="${esc(o.id)}">Accept + Build Replacement Route</button>`:''}${canWithdraw?`<button type="button" data-offer-withdraw="${esc(o.id)}">Withdraw</button>`:`<button type="button" data-offer-remove="${esc(o.id)}">Remove from history</button>`}</div></article>`}).join(''):`<p>No Chairman's Offers yet. Aurora creates one at +${AUTO_GAIN_TRIGGER_PCT}% only when the replacement improves annual income by at least ${MIN_INCOME_UPLIFT_PCT}% and is buyable in the same broker account.</p>`;
  ...
  function boot(){const A=window.AuroraClean;if(!A){setTimeout(boot,60);return}if(!A.readState().squad?.holdings?.length)A.importRealHoldings?.();$('chairmanHoldingSelect')?.addEventListener('change',syncSelectedHolding);$('chairmanCreateOffer')?.addEventListener('click',createOffer);$('chairmanRefreshHoldings')?.addEventListener('click',refreshHoldings);window.addEventListener('aurora-clean:state',refreshAll);window.addEventListener('aurora:market-prices',refreshAll);refreshAll();setTimeout(refreshHoldings,300);window.AuroraChairmanOffers=Object.freeze({BUILD,AUTO_GAIN_TRIGGER_PCT,MIN_INCOME_UPLIFT_PCT,COOLDOWN_DAYS,render,reconcile,autoGenerateOffers,bestReplacement,brokerEligible,createOffer,refreshHoldings})}
