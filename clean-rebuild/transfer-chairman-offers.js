(() => {
  ...
  const BUILD='20260827-clean-chairman-offers-1';
  ...
  function reconcile(){
    ...
    const state=A.readState(),rows=offers(state);let changed=false;
    rows.forEach(o=>{if(upper(o.status)!=='WATCHING')return;const m=offerMetrics(state,o);if(m.live>0&&m.target>0&&m.live>=m.target){o.__activate=true;changed=true}});
    if(!changed)return;
    A.updateState(next=>{(next.transfer.chairmanOffers||[]).forEach(o=>{if(o.__activate&&upper(o.status)==='WATCHING'){o.status='ACTIVE';o.activatedAt=new Date().toISOString();o.lastLivePrice=round(findHolding(next,o)?.livePriceGbp||o.lastLivePrice);delete o.__activate}})});
  }
  ...
  function boot(){const A=window.AuroraClean;if(!A){setTimeout(boot,60);return}if(!A.readState().squad?.holdings?.length)A.importRealHoldings?.();$('chairmanHoldingSelect')?.addEventListener('change',syncSelectedHolding);$('chairmanCreateOffer')?.addEventListener('click',createOffer);window.addEventListener('aurora-clean:state',()=>{reconcile();render()});window.addEventListener('aurora:market-prices',()=>{reconcile();render()});reconcile();render();window.AuroraChairmanOffers=Object.freeze({BUILD,render,reconcile,createOffer})}
