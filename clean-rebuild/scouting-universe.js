(() => {
  ...
  const MASTER_URL = '../AuroraMaster.json?v=20260826-clean-universe-2';
  ...
  function normaliseRow(row, source) {
    const ticker = upper(cell(row,[
      'ticker','symbol','yahoo symbol','yahoo_symbol','lse symbol','lse_symbol','epic','code'
    ]));
    if (!ticker) return null;

    const name = String(cell(row,['name','company','company name','company_name']) || ticker).trim();
    const liveNative = num(cell(row,[
      'live_price_native','live price native','native price','live_price','live price','price','current price','current_price'
    ]));
    const dpsNative = num(cell(row,[
      'annual_dps_native','annual dps native','native dps','annual_dps','annual dps','dps','annual dividend per share','dividend per share'
    ]));
    ...
    const sheetYield = num(cell(row,[
      'yield_pct','yield pct','yield','dividend yield','dividend_yield','forward yield','forward_yield'
    ]));
    const yieldPct = livePrice > 0 && annualDps > 0
      ? (annualDps / livePrice) * 100
      : (sheetYield > 0 && sheetYield < 1 ? sheetYield * 100 : sheetYield);
    const buyStrength = Math.max(0, Math.min(100, num(cell(row,[
      'buy_strength','buy strength','score','watch pressure','watch_pressure'
    ]))));
  ...
  async function fetchPools() {
    ...
    try {
      ...
      if (Object.values(pools).some(rows => rows.length)) {
        return {pools, source:'AuroraMaster.json'};
      }
      throw new Error('AuroraMaster.json contained no scouting universe tabs');
    } catch (error) {
      masterError = error;
    }
    ...
    if (Object.values(pools).some(rows => rows.length)) {
      return {pools, source:'OpenSheet fallback'};
    }
    throw masterError || new Error('No Aurora scouting universe source returned rows');
  }
  ...
  async function refresh() {
    ...
    try {
      ...
      setStatus(`Loaded ${unique} unique candidates · Watchlist ${counts.watchlist} · Global ${counts.global} · Scout ${counts.scout} · ${source}`);
    } catch (error) {
