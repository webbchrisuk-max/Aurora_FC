(() => {
  ...
  const BUILD = '20260831-clean-house-baseline-2-user-rooms';
  ...
  function apply(){
    ...
    try { if(localStorage.getItem(BASELINE_KEY) === 'done') return true; } catch(_) {}
    ...
    A.updateState(state => {
      ...
      if(!pot){ pot = {id:'house_fund',name:'House Fund'}; state.finance.pots.push(pot); }
      Object.assign(pot, {
        id:'house_fund', name:'House Fund', balance:18771.14, target:20436.66, spent:3497.45,
        goalMode:'funded-progress', deadline:'', fundingOverride:0, priority:2, archived:false,
  ...
  function boot(){
    if(apply()) return;
    let tries=0;
    const timer=setInterval(() => { tries += 1; if(apply() || tries > 200) clearInterval(timer); },25);
  }
